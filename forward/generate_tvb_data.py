...
logging.getLogger("tvb.basic").setLevel(logging.ERROR)
    ...
    for iter_a in range(A.shape[1]):
        ...
        for iter_m in range(mean_and_std.shape[1]):
            ...
            for iii in range(2):
                ...
                for alias, original in REGION_REMAPPING.items():
                    data[:, alias] = data[:, original]
                data = data[:, :994]
...
if __name__ == "__main__":
    ...
    processes = [
        mp.Process(target=main, args=(x,)) for x in range(args.a_start, args.a_end)
    ]
    for p in processes:
        p.start()
