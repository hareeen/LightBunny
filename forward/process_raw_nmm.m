...
addParameter(p, 'regions', 1:1, @isnumeric);
parse(p, varargin{:})
...
% -------------------------------------------------------------------------
iter_list = 0:0;   % the iter during NMM generation.
...
for i_iter = 1:length(iter_list)
    ...
    remaining_regions = p.Results.regions;
    ...
    for ii = 1:length(remaining_regions)                                 % Change iteration to the num of NMM regions you want to generate
        ...
        if ~isempty(spike_time)
            fprintf('spike_time: [%s]\n', join(string(spike_time), ','));
        ...
        fprintf(['iter_' int2str(iter) '_i_%d is done\n'], i-1)
    end % END REGION
end % END ITER
end % ENG FUNCTION
